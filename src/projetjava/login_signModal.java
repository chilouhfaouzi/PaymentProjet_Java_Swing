...
public class login_signModal {
    ...
    public boolean registeruser() throws SQLException, ClassNotFoundException {
        ...
        if (row > 0) {
            try (Statement statm = con.createStatement()) {
                PreparedStatement verlogin =
                        con.prepareStatement(string_sql2);
                verlogin.setString(1, email);
                verlogin.setString(2, password);
                try (ResultSet res = verlogin.executeQuery()) {
                    boolean b = res.last();
                    int count = res.getRow();
                    res.beforeFirst();
                    if (count > 0) {
                        ...
                        while (res.next()) {
                            int id = res.getInt("id");
                            String addcompte = "INSERT INTO `compte` ( `id_user`, `Solde`) VALUES ( ?,  " +
                                    "'100');";
                            PreparedStatement addcomptest =
                                    con.prepareStatement(addcompte);
                            addcomptest.setInt(1, id);
                            int row2 = addcomptest.executeUpdate();
    ...
    public boolean checkphone(int id) throws ClassNotFoundException {   // select from data base
     ...
     * @author Amine NAFID
     * *******************************/
    public boolean chekCardNumber(String card_num, String cvv, String date_exp) throws ClassNotFoundException,
            SQLException {

        try {
            // Set Connection
            Connection con = ConnectionDB.my_connect();
            PreparedStatement sql = con.prepareStatement("SELECT * FROM card WHERE card_number = ? AND cvv=? AND " +
                    "date_expiration=?");
